...
'''
※pip install firebase_admin 필수※

1.0.1 패치내용
    UI,창 이름 수정

1.1 패치내용
    가로 세로 축 바꿈(Left_d, Down_d) 버그 패치
    게임 속도 향상
    서버 자동 초기화
    자기 차례일때 좌측 상단에 표기
'''
...
Your_piece_xy=-Sizeofsqare*1.6-Sizeofsqare2*3.3-Left_d, Sizeofsqare*2-Sizeofsqare2*1.1-Down_d
...
def Screen(T):
    ...
    T.ht()
    T.speed(0)
    T.goto(-Sizeofsqare*1.5-Left_d, -Sizeofsqare*2-Down_d)
    T.begin_fill()
    Draw_sqare(Sizeofsqare*3, Sizeofsqare, T, Pensize=1, Text='', Font='', Fontsize=80, Color='#B1CDFC')
    T.end_fill()
    T.goto(-Sizeofsqare*1.5-Left_d, Sizeofsqare-Down_d)
    T.begin_fill()
...
def Start(Turn, Y, X):
    ...
    if K.Name=='자': #자가 끝까지 도달하면 후로 바꿈
        if (Turn and Y==0) or (not Turn and Y==3):
            K.Name='후'
            K.Realname='侯'
    K.T.clear() #모니터 시각화
...
def Piece_go(Name, Y, X, Turn):
    if Name=='자':
        ...
        K=[(Y+i,X+j) for i in [-1,0,1] for j in [-1,0,1] if not(i==0 and j==0)]
        K.remove((Y+(1 if Turn else -1),X-1))
        K.remove((Y+(1 if Turn else -1),X+1))
        ...
        #포로 선택할 때
            ...
            if Sizeofsqare*1.6<=X+Left_d<=Sizeofsqare*1.6+Sizeofsqare2*3.4 and -Sizeofsqare*2<=Y+Down_d<=-Sizeofsqare*2+Sizeofsqare2*2.3:
                for i in range(len(My_piece)):
                    ...
                    Start_x, End_x=My_piece_x+Sizeofsqare2*(i%3)*1.1, My_piece_x+Sizeofsqare2*(i%3*1.1+1)
                    Start_y, End_y=My_piece_y+(0 if 0<=i<=2 else -Sizeofsqare2*1.1), My_piece_y+Sizeofsqare2*(1 if 0<=i<=2 else -.1)
            ...
            if -Sizeofsqare*1.6-Sizeofsqare2*3.4<=X<=-Sizeofsqare*1.6 and Sizeofsqare*2-Sizeofsqare2*2.3<=Y<=Sizeofsqare*2:
                for i in range(len(Your_piece)):
                    ...
                    Start_x, End_x=Your_piece_x+Sizeofsqare2*(i%3)*1.1, Your_piece_x+Sizeofsqare2*(i%3*1.1+1)
                    Start_y, End_y=Your_piece_y+(0 if 0<=i<=2 else -Sizeofsqare2*1.1), Your_piece_y+Sizeofsqare2*(1 if 0<=i<=2 else -.1)
                    if Start_x<=X<=End_x and Start_y<=Y<=End_y:
                        break
                else:
                    Dont_click=0
...
def Loading():
    ...
    Checkmate(not Turn) if Choice_x==-1 else Checkmate(not Turn, Y, X)
    turtle.title('십이장기 - 내 차례')
